...
public class Solution
{
    ...
    public static void sort(List<Stock> list)
    {
        Collections.sort(list, new Comparator<Stock>()
        {
            ...
            {
                int compareName = stock1.get("name").toString().compareTo(stock2.get("name").toString());
                if (compareName != 0)
                    return compareName;
                else
                {
                    ...
                    else
                    {
                        ...
                        if (stock1.containsKey("change"))
                            one = (Double) stock1.get("change");
                        else
                            one = (Double) stock1.get("last") - (Double) stock1.get("open");

                        Double two;
                        if (stock2.containsKey("change"))
                            two = (Double) stock2.get("change");
                        else
                            two = (Double) stock2.get("last") - (Double) stock2.get("open");

                        int compareChange = two.compareTo(one);
                        if (compareChange != 0)
